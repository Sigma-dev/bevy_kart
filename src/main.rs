...
fn main() {
    App::new()
        ...
        .init_resource::<PreRollbackState>()
...
/// Save Position/Rotation of all smoothed entities before rollback can happen.
...
/// After rollback+replay (PreTick) but before tick advance, detect position corrections
/// and absorb them into the visual offset. Also saves prev position for interpolation.
fn detect_corrections(
    ...
    for (entity, pos, rot, mut smoothing) in karts.iter_mut() {
        ...
        // Save as prev for sub-tick interpolation (before Tick advances Position)
